          ...
          <div class="border-t border-gray-100 py-4 px-6 bg-gradient-to-r from-white to-emerald-50 rounded-b-lg">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <div class="flex items-center gap-1">
                ...
                >
                  Next
                  <ChevronRight class="w-4 w-4 ml-1" />
                </button>
              </div>
            </div>
          </div>
...
const fetchTempHumidityData = async () => {
  try {
    ...
    const fetchPromise = getDocs(tempQuery)
    const timeoutPromise = new Promise((_, reject) => 
      setTimeout(() => reject(new Error('Fetch timeout')), 10000)
    )
    
    const tempSnapshot = await Promise.race([fetchPromise, timeoutPromise])
    
    // Process temperature and humidity readings
    const processedData = tempSnapshot.docs
      .filter(doc => {
        const data = doc.data()
...
const setupRealtimeListener = () => {
  ...
  function processSnapshot(snapshot) {
    ...
    if (newData.length > 0) {
      ...
      const tempValues = newData.map(item => item.temperature)
      tempStats.value = {
      ...
      const humidityValues = newData.map(item => item.humidity)
      humidityStats.value = {

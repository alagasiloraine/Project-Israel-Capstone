<template>
            ...
            <thead>
              <tr class="bg-gray-200">
                <th 
                  ...
                  :class="{ 'cursor-pointer hover:bg-gray-300': header.key === 'waterLevel' }"
                  @click="header.key === 'waterLevel' ? sortByWaterLevel() : null"
                >
                  <div class="flex items-center space-x-1">
                    <span>{{ header.label }}</span>
                    <ArrowUp v-if="header.key === 'waterLevel'" class="h-4 w-4" />
                  </div>
                </th>
              </tr>
            </thead>
  ...
  const data = ref([
    ...
    { id: 4, waterLevel: 78, date: '2024-05-17', time: '19:00:25' },
  ])
  
  const searchQuery = ref('')
  const isWaterLevelSorted = ref(false)
  
  const sortByWaterLevel = () => {
    isWaterLevelSorted.value = true
  }
  ...
  const sortedData = computed(() => {
    ...
    if (isWaterLevelSorted.value) {
      filteredData.sort((a, b) => a.waterLevel - b.waterLevel)
    }

<template>
      ...
      <div class="flex-1 p-6 overflow-auto">
        ...
        <div class="flex justify-between items-center mb-6">
          ...
          <button class="p-2 rounded-lg hover:bg-gray-200 transition-colors">
            <Download class="h-5 w-5 text-red-600" />
          </button>
        </div>
            ...
            <thead>
              <tr class="bg-gray-200">
                <th 
                  ...
                  :class="{ 'cursor-pointer hover:bg-gray-300': header.key === 'temperature' }"
                  @click="header.key === 'temperature' ? sortByTemperature() : null"
                >
                  <div class="flex items-center space-x-1">
                    <span>{{ header.label }}</span>
                    <ArrowUp v-if="header.key === 'temperature'" class="h-4 w-4" />
                  </div>
                </th>
              </tr>
            </thead>
  ...
  const data = ref([
    { id: 1, temperature: 25, date: '2024-05-17', time: '18:58:33' },
    { id: 2, temperature: 26, date: '2024-05-17', time: '18:58:48' },
    { id: 3, temperature: 24, date: '2024-05-17', time: '18:59:24' },
    { id: 4, temperature: 25, date: '2024-05-17', time: '19:00:25' },
  ])
  
  const searchQuery = ref('')
  const isTemperatureSorted = ref(false)
  
  const sortByTemperature = () => {
    isTemperatureSorted.value = true
  }

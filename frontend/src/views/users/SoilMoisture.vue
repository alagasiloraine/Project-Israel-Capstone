<template>
            ...
            <thead>
              <tr class="bg-gray-200">
                <th 
                  ...
                  :class="{ 'cursor-pointer hover:bg-gray-300': header.key === 'soilMoisture' }"
                  @click="header.key === 'soilMoisture' ? sortBySoilMoisture() : null"
                >
                  <div class="flex items-center space-x-1">
                    <span>{{ header.label }}</span>
                    <ArrowUp v-if="header.key === 'soilMoisture'" class="h-4 w-4" />
                  </div>
                </th>
              </tr>
            </thead>
  ...
  const isSoilMoistureSorted = ref(false)
  
  // Sort only by soil moisture in ascending order
  const sortBySoilMoisture = () => {
    isSoilMoistureSorted.value = true
  }
  ...
  const sortedData = computed(() => {
    ...
    if (isSoilMoistureSorted.value) {
      filteredData.sort((a, b) => a.soilMoisture - b.soilMoisture)
    }

<template>
  <div class="flex h-screen overflow-hidden bg-gray-50">
    <!-- Sidebar with updated background color to match the image -->
    <div class="w-96 bg-gradient-to-b from-[#002E1D] to-[#002E1D]/90 text-white relative rounded-r-3xl shadow-xl">
      <!-- User Profile -->
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div class="relative group">
              ...
              <span class="absolute -top-1 -right-1 w-4 h-4 bg-[#002E1D] rounded-full border-2 border-white/30"></span>
            </div>
            <div class="flex-1">
          ...
          <div class="flex gap-2">
            ...
            <div class="relative">
              ...
              <div v-if="showSettingsDropdown" 
                   class="absolute right-0 mt-2 w-48 bg-[#002E1D] rounded-lg shadow-lg p-2 z-50">
      ...
      <div class="p-4 border border-white/10 rounded-lg mx-4 mt-4 bg-white/5">
        <div class="flex items-center justify-between mb-2">
          <span class="text-sm text-white/70">Task for Today</span>
        </div>
        <h3 class="font-medium mb-2">{{ currentTask.title }}</h3>
        <div class="flex items-center gap-2 mb-2">
          ...
          />
          <div>
            <p class="text-sm font-medium">Staff 1 - Profile</p>
            <p class="text-xs text-white/50">Staff 1</p>
          </div>
        </div>
        <p class="text-sm text-white/50 mb-3">12:00 - 13:30</p>
        <div class="flex gap-2">
          <button 
            @click="showEventDetails = true"
            class="px-4 py-1 bg-white/10 hover:bg-white/20 rounded-md text-sm transition-colors"
          >
            Details
          </button>
        </div>
        ...
        <div v-show="showTasks" class="space-y-3">
          <div v-for="task in tasks" :key="task.id" class="flex items-center gap-3 cursor-pointer group">
            <input 
              ...
              class="rounded border-white/30 text-[#002E1D] focus:ring-[#002E1D] bg-white/10" 
            />
            <span :class="{ 'line-through': task.completed }">{{ task.title }}</span>
            <div class="flex items-center ml-auto">
              <span 
                class="w-5 h-5 rounded-full block mr-2"
                :class="task.completed ? 'bg-[#002E1D]' : 'bg-[#FFA500]'"
              ></span>
    ...
    <div class="flex-1 flex flex-col h-screen bg-white">
        ...
        <div class="flex flex-col space-y-6">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-2">
              ...
              <button class="px-4 py-2 rounded-lg bg-[#002E1D] text-white hover:bg-[#002E1D]/90 transition-colors" @click="goToToday">
                Today
              </button>
              <button class="p-2 rounded-lg bg-gray-100 hover:bg-gray-200 transition-colors" @click="navigateDate(1)">
          ...
          <div class="flex items-center">
            ...
            <div class="grid grid-cols-7 gap-4 flex-1">
              <div 
                ...
                :class="{'bg-[#002E1D] text-white': selectedDay === day.date, 'hover:bg-[#FFA500] hover:text-white': selectedDay !== day.date}"
                @click="selectDay(day)"
              >
      ...
      <!-- Time Grid -->
      <div class="grid grid-cols-[60px_1fr] divide-x h-[600px] overflow-y-auto">
        <!-- Time Labels -->
        <div class="divide-y">
          <div 
        ...
        <div class="divide-y">
          ...
          >
            ...
            >
              ...
              <div 
                class="absolute inset-1 border-2 border-dashed border-gray-200 rounded-xl 
                       opacity-0 group-hover:opacity-100 group-hover:border-[#002E1D] transition-all duration-200
                       flex items-center justify-center cursor-pointer"
                @click="openAddEventModal(hour, day)"
              >
                <PlusIcon class="w-4 h-4 text-gray-400 group-hover:text-[#002E1D]" />
              </div>
              ...
              <template v-for="event in getEventsForTimeSlot(hour, day)" :key="event.id">
                <div 
                  ...
                  class="absolute left-1 right-1 rounded-xl p-2 border-l-4 flex flex-col gap-1"
                >
    ...
    >
      <div class="bg-white rounded-xl p-6 w-full max-w-md text-gray-900">
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <div v-else class="space-y-2">
              ...
              <button 
                ...
                class="text-sm text-[#002E1D] hover:text-[#002E1D]/80"
              >
                + Add Staff
              </button>
            </div>
          ...
          <div class="flex justify-end gap-2">
            <button 
              v-if="!isEditing"
              @click="startEditing"
              class="px-4 py-2 bg-[#002E1D] text-white rounded-lg hover:bg-[#002E1D]/90 transition-colors"
            >
              Edit Details
            </button>
            <template v-else>
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-2 p-2 border rounded-lg">
              ...
              >
                ...
                />
                <span class="text-sm">{{ staff.name }}</span>
                <button @click="removeStaff(staff.id)" class="text-gray-500 hover:text-gray-700">
                  <XIcon class="w-4 h-4" />
                </button>
              </div>
              <button 
  ...
  const addStaffMember = (staff) => {
    if (isEditing.value) {
      if (!editingTask.value.staff.find(s => s.id === staff.id)) {
        editingTask.value.staff.push(staff)
      }
    } else {
  ...
  const getEventHeight = (event) => {
    return event.duration * 64 - 8 // Updated for new height (h-16 = 64px)
  }
